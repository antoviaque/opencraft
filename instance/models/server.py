...
class Server(ValidateModelMixin, TimeStampedModel):
    ...
    _status_to_terminated = status.transition(to_state=Status.Terminated)  # FIXME: Restrict from_states
...
class OpenStackServer(Server):
    ...
    def terminate(self):
        ...
        if self.status == Status.Terminated:
            return
        elif self.status == Status.Pending:  # FIXME: We might not need this
